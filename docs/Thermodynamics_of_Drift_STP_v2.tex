...
\subsection*{Implementation note: The Prompt Layer (normative)}
To make the microservice boundaries unambiguous, STP v2.0 separates \emph{proposal} from \emph{acceptance}:

\begin{itemize}
  \item \textbf{Governor:} a deterministic policy engine (state machine). It may call the Auditor to compute drift signals, applies thresholds/budgets, and emits \texttt{TRANSMIT\_ACK}, \texttt{NACK:BRIDGE}, or \texttt{FIN}.
  \item \textbf{Auditor:} cheap and fast (embedding model or small verifier). It computes $(d_{\mathrm{sem}}, d_{\mathrm{vibe}})$ and returns diagnostics; it does not rewrite content.
  \item \textbf{Host:} the generative model. It \emph{only} proposes candidate drafts and Bridge Artifacts. The Host never decides acceptance; all acceptance is mediated by the Governor via the protocol.
\end{itemize}

This prompt-layer separation enables independent tuning (cost/latency) and prevents a single model from both \emph{proposing} and \emph{ratifying} its own outputs.
